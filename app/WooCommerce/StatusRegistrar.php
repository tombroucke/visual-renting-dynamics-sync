...
class StatusRegistrar
{
    private function mergeDefault(array $customStatus) : array
    {
        $defaultArgs = [
            ...
            'label_count'               => _n_noop('%s <span class="count">(%s)</span>', '%s <span class="count">(%s)</span>')
        ];
    ...
    public function add(string $customStatusName, array $customStatus) : self
    {
        ...
        add_filter('wc_order_statuses', function($orderStatuses) use ($customStatusName, $customStatus) {
        ...
        add_filter('bulk_actions-woocommerce_page_wc-orders', function($bulkActions) use ($customStatusName, $customStatus) {
            return $this->addOrderStatuses(
                $bulkActions, 
                $customStatusName, 
                sprintf(__('Change status to %s', 'visual-renting-dynamics-sync'), Str::lower($customStatus['label']))
            );
        }, 20, 1);

        add_action('handle_bulk_actions-woocommerce_page_wc-orders', function($redirect, $action, $postIds) use ($customStatusName) {
    ...
    public function updateOrderStatus($customStatusName, $postIds) : void
    {
