...
class ArticleSyncService implements Runnable
{
    ...
    public function run(array $args, array $assocArgs): void
    {
        ...
        $this->api->articles()
            ->filter(fn ($article) => $article['publicerenInternet'])
            ...
            ->each(
                function ($article) use ($syncer, $assocArgs) {
                    $args = [
                        ...
                        'woocommerce' => [
                            ...
                            'meta_input' => [
                                ...
                                '_length' => $article['lengte'] !== '' ? $article['lengte'] * 100 : '',
                                '_width' => $article['breedte'] !== '' ? $article['breedte'] * 100 : '',
                                '_height' => $article['hoogte'] !== '' ? $article['hoogte'] * 100 : '',
                    ...
                    collect($article['tags'])
                        ->each(fn ($tag) => $args['meta_input']['tags'][] = $tag['tag']);
                    ...
                    if (! $assocArgs['skip-images']) {
                        collect([
                            ...
                            ->filter(fn ($imageKey) => $article[$imageKey])
                            ->each(
                                function ($imageKey, $key) use ($article, &$media) {
                                    ...
                                    $media[] = [
                                        ...
                                        'filestream' => fn () => $imageResponse->getBody()->getContents(),
                                        'filename' => $filename,
                        ...
                        ->each(
                            function ($document, $key) use (&$media) {
                                ...
                                $media[] = [
                                    ...
                                    'filestream' => fn () => $documentResponse->getBody()->getContents(),
                                    'filename' => $filename,
                                    'title' => strtok(pathinfo($filename, PATHINFO_FILENAME), '?'),
